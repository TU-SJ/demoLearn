...
import queue.service.抢红包.ISeckillDistributedService;
...
import zookeeper.Zookeeper的Util;
...
public class SeckillDistributedServiceImpl implements ISeckillDistributedService {
    ...
    public Result startSeckilRedisLock(long seckillId, long userId) {
        ...
        try {
             ...
             * 总结一下为什么会超卖101：秒杀开始后，某个事物在未提交之前，锁已经释放(事物提交是在整个方法执行完)
             * 导致下一个事物读取到了上个事物未提交的数据，也就是传说中的脏读。
             * ==========================================================================================
             ...
             * 所以这钟实现也是错误的，事物和锁会有冲突，建议AOP实现。AOP保证了在代码的point.process执行完才释放锁
             */
    ...
    public Result startSeckilZksLock(long seckillId, long userId) {
        ...
        try {
            ...
            res = Zookeeper的Util.acquire(3,TimeUnit.SECONDS);
            if(res){
                String nativeSql = "SELECT number FROM seckill WHERE seckill_id=?";
                Object object =  dynamicQuery.nativeQueryObject(nativeSql, new Object[]{seckillId});
                Long number =  ((Number) object).longValue();
