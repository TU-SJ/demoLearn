...
public class 红包延迟队列过期消费 implements ApplicationRunner {
    private final static Logger LOGGER = LoggerFactory.getLogger(红包延迟队列过期消费.class);
   ...
   @Autowired
    ...
    public void run(ApplicationArguments var){
        executorService.execute(() -> {
            while (true) {
                try {
                    RedPacketMessage message = 红包延迟队列2.getQueue().consume();
                    if(message!=null){
